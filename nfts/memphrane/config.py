    ...
    for dir_name in entries:
        for file_name in os.listdir(path + '/' + dir_name):
            elements.append({
                ...
                'value': path + '/' + dir_name + '/' + file_name
            })
    return elements


def get_model_elements(_path, _rarity):
    return [{'id': 0, 'value': 'Humanoid1', 'name': 'Humanoid 1', 'weight': 100},
            {'id': 1, 'value': 'Humanoid2', 'name': 'Humanoid 2', 'weight': 100},
            {'id': 2, 'value': 'Humanoid3', 'name': 'Humanoid 3', 'weight': 100} ]
...
def add_variation(_variation_id, _variation_type):
    ...
    for i in range(len(rarity_weights)):
        ...
        elements_for_rarity = get_elements(
            assets_dir + _variation_id, rarity, _variation_type)
